...
4.  **Run Automated Visual Diff**
    ...
    - Command:
      ...
      ```
    - The script will output the percentage of mismatched pixels, count of discrepancies, and exit with `1` if there are significant errors. It will also generate `diff_result.png` with red bounding boxes drawn over the incorrect areas.

5.  **Iterate and Fix**
    - If the script returns an error, use `read_file` to view `diff_result.png`.
    - Observe where the red bounding boxes are. This tells you exactly what is misaligned, wrongly sized, or missing.
